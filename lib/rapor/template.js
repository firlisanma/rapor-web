...
<table class="no-border">
  ...
  <tr>
    ...
    <td>Semester</td><td>: ${semesterText}</td>
  </tr>
  <tr>
...
<table class="nilai">
  <thead>
    <tr>
      ...
      <th rowspan="2">Rata-Rata<br/>Nilai Kelas</th>
    </tr>
    <tr>
      <th>Kode</th>
      <th>Urut</th>
  ...
  <tbody>
    ${Object.entries(kelompok).map(([kode, kel]) => `
      <tr class="kelompok">
   <td class="center" rowspan="${kel.rows.length + 1}">
      ${kode}
  <td></td>
  <td colspan="5"><b>(Mata Pelajaran Wajib)</b></td>
</tr>

       ${kel.rows.map(r => `
    <tr>
      <td class="center">${r.urut}</td>
      <td>${r.nama_mapel}</td>
      <td class="center">${r.kkm}</td>
      <td class="center">${r.nilai_angka ?? "-"}</td>
      <td>${r.nilai_huruf ?? "-"}</td>
      <td class="center">-</td>
    </tr>
  `).join("")}
`).join("")}
    ...
    <!-- JUMLAH & RATA-RATA (sementara) -->
    <tr class="summary">
      <td colspan="4"><b>Jumlah Nilai</b></td>
      <td class="center">-</td>
      <td colspan="2"></td>
    </tr>
    <tr class="summary">
